...
class Region(Entity):
    ...
    srid = Field(Integer)
    units = Field(String)
    earth_circumference = Field(Float)
    block_length = Field(Float)
    jog_length = Field(Float)
    edge_attrs = OneToMany('EdgeAttr', order_by='id')
    geocodes = OneToMany('Geocode')
    routes = OneToMany('Route')
    ads = OneToMany('Ad')
...
class EdgeAttr(Entity):
    ...
    name = Field(String)
    region = ManyToOne('Region', **cascade_args)
    def __repr__(self):
...
class Service(Entity):
    title = Field(String)
    region = ManyToOne('Region', inverse='geocodes', **cascade_args)
