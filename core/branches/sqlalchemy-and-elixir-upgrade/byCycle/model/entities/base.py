...
class Node(Entity):
    geom = Field(POINT(4326))
    edges_f = OneToMany('Edge', inverse='node_f')
    edges_t = OneToMany('Edge', inverse='node_t')
...
class Edge(Entity):
    addr_f_l = Field(Integer)
    addr_f_r = Field(Integer)
    addr_t_l = Field(Integer)
    addr_t_r = Field(Integer)
    even_side = Field(CHAR(1))
    one_way = Field(Integer)
    permanent_id = Field(Integer)
    geom = Field(LINESTRING(4326))
    node_f = ManyToOne('Node', **cascade_args)
    node_t = ManyToOne('Node', **cascade_args)
    street_name = ManyToOne('StreetName', **cascade_args)
    place_l = ManyToOne('Place', **cascade_args)
    place_r = ManyToOne('Place', **cascade_args)
...
class StreetName(Entity):
    prefix = Field(String(2))
    name = Field(String)
    sttype = Field(String(4))
    suffix = Field(String(2))
...
class State(Entity):
    code = Field(CHAR(2))  # Two-letter state code
    state = Field(String)
...
class Place(Entity):
    zip_code = Field(Integer)
    city = ManyToOne('City', **cascade_args)
    state = ManyToOne('State', **cascade_args)
