...
<html xmlns="http://www.w3.org/1999/xhtml"
  ...
  <head>
    ...
    <!--[if IE]>
	<% h.stylesheet_link_tag('base_ie.css') %>
    <![endif]-->
    <% h.stylesheet_link_tag('%s.css' % c.region_key) %>
    ...
    <% h.javascript_include_tag('bycycle.js') %>
    <% h.javascript_include_tag('util.js') %>
  ...
  <body>
	...
	<div id="logo_pane" dojoType="ContentPane" layoutAlign="left">
	  <a href="<% h.url_for('/') %>" title="byCycle Trip Planner"><img src="<% h.url_for('/images/logo.png') %>" width="93" height="66" /></a>
	</div>
	<!-- End Logo Pane -->
	...
	<div id="input_container" dojoType="TabContainer" layoutAlign="client">
	  ...
	  <div id="query_pane" dojoType="ContentPane" label="Search Map">
	    <form id="query_form" action="<% h.url_for('/%s/query/' % c.region_key) %>" method="get" onsubmit="byCycle.UI.runQuery(this); return false;">
	    <div>
	...
	<div id="route_pane" dojoType="ContentPane" label="Find Route">
	  <form id="route_form" action="<% h.url_for('/%s/route/' % c.region_key) %>" method="get" onsubmit="new byCycle.UI.RouteQuery(this).run(); return false;">

	  <div>
	    <!-- This is laid out like this intentionally to
		 control the display of these elements. -->
	    <input id="s" name="s" type="text" value="<% c.s %>" title="Enter start address" tabindex="4"
	    /><a
	    id="swap_s_and_e"
  ...
  <div id="content_pane" dojoType="LayoutContainer" layoutAlign="client">
	...
	<div id="spinner" style="display: none;">
	  <img src="<% h.url_for('/images/spinner.gif') %>" width="16" height="16" />
	</div>
...
% #if
	...
	<div id="result_container" dojoType="TabContainer"
	  ...
	  <div id="location_list_pane" dojoType="ContentPane" label="Locations">
	    <ul id="location_list" class="result_list"></ul>
	  </div>

	  <div id="route_list_pane" dojoType="ContentPane" label="Routes">
	...
	<!-- Map -->
	<div id="map_pane" dojoType="ContentPane" layoutAlign="client"></div>
	<!-- End Map -->
...
<% h.javascript_include_tag('regions.js') %>
<% h.javascript_include_tag('map.js') %>
<% h.javascript_include_tag('gmap.js') %>
<% h.javascript_include_tag('ui.js') %>
<% h.javascript_include_tag('result.js') %>
